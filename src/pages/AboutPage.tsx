...
import { useGetTestimonialsQuery, type Testimonial } from "../app/api/testimonials";
import { Button, PageSection } from "../components";

function TestimonialCard({ t }: { t: Testimonial }) {
  return (
    <article className="flex-shrink-0 w-80 flex flex-col gap-4 rounded-lg border border-secondary-300/20 bg-secondary-200 p-6 shadow-[0_12px_30px_rgba(0,0,0,0.06)]">
      ...
      <p className="flex-1 text-sm leading-7 text-secondary-100 line-clamp-4">"{t.testimonial}"</p>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {(t.clientPosition || t.clientCompany) && (
            <p className="text-xs text-secondary-300">{[t.clientPosition, t.clientCompany].filter(Boolean).join(', ')}</p>
          )}
        </div>
      </div>
    </article>
