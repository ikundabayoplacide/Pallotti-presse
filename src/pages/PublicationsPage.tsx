...
import { HiBookOpen, HiCalendar, HiEye } from "react-icons/hi2";
...
function handleDownload(pub: Publication) {
  if (!pub.fileContent) return;
  try {
    // fileContent is a data URI like "data:application/pdf;base64,..."
    const [meta, base64] = pub.fileContent.split(",");
    ...
    a.href = url;
    a.download = pub.fileName || pub.title;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  } catch {
    window.open(pub.fileContent, "_blank");
  }
}

function PublicationCard({ pub }: { pub: Publication }) {
  const isFileBased = Boolean(pub.fileContent);
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-lg border border-secondary-300/20 bg-secondary-200 shadow-[0_12px_30px_rgba(0,0,0,0.04)] transition-all hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(0,0,0,0.10)]">
      ...
      <div className="relative h-48 overflow-hidden bg-primary-800/10">
        ...
        {pub.featured && (
          <span className="absolute top-3 left-3 rounded-full bg-primary-700 px-3 py-1 text-xs font-semibold text-secondary-200">
            Featured
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h3 className="mb-2 text-lg font-semibold leading-snug text-secondary-100 line-clamp-2">
          {pub.title}
        </h3>
        <p className="mb-4 flex-1 text-sm leading-6 text-secondary-300 line-clamp-3">
          {getExcerpt(pub)}
        </p>
      ...
      <section className="bg-linear-to-r from-primary-800 via-custom-800 to-primary-700">
        <div className="mx-auto max-w-7xl px-4 py-10 text-center xxs:px-5 xs:px-6 sm:px-8 lg:px-6 lg:py-14">
          <div className="mx-auto max-w-3xl space-y-4">
            <h1 className="text-4xl font-semibold leading-tight text-primary-100 xxs:text-5xl md:text-6xl">
              Publications
            </h1>
            <p className="text-lg leading-7 text-primary-100">
              Browse our latest publications, reports, and printed materials.
            </p>
          </div>
        </div>
      </section>
      ...
      <PageSection className="bg-secondary-200">
        ...
        {!isLoading && !isError && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="mt-2 text-sm text-secondary-300">
              {activeCategory !== "All"
                ? `No publications in the "${activeCategory}" category yet.`
